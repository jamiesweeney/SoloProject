...
\begin{abstract}
The purpose of this paper is design and implement a room ocupancy prediction system, through use of smart sensor boxes. We propose and attempt to build a system that monitors the predictions of University of Glasgow owned buildings.
\end{abstract}
...
Advanced examples of this are already taking concrete form, with the University of Glasgow investing \euro 1bn towards developing a smart campus\cite{c-iot-uog} and the city of Barcelona making major overhauls to many public services in an attempt to bring the idea of a smart city to reality\cite{c-iot-bar}.
...
Another sign of human occupancy could be room temperature, as occupants naturally heat the air through emission of body heat. However the standard precision of an appropriate temperature sensor is usually 0.5°C \cite{c-temp}, which is too low to be useful in determining exact temperature reading. The temperature change undergone when a group of people enter the room is unlikely be high enough to be properly detected by the sensor and be a guaranteed indication of human occupancy. This is especially true when the room temperature is subject to external factors, for example sun shining through the window will cause the room temperature to increase rapidly. We find that it would be too difficult to separate such an event from temperature rises that indicate human activity, so it follows that temperature sensors would be of little use for this problem.
...
Although the aim of this system is to detect human occupancy, every day, more and more people are using smartphones or similar electronic wireless devices for almost every aspect of life. It is estimted that 83\% of adults in the UK own a smartphone (as of 2016)\cite{c-phones}, and with most of these people using their devices everywhere they go, it is reasonable to say that the number of wireless devices in a specific area would be very highly correlated with the number of human occupants. We must also consider the possibility of rooms containing wireless devices that are not relative to the human occupants, for example lab computers may contain Bluetooth functionality that would distort the human to device ratio, however typical workplace machines operate using wired communication, as it is more practical for a stationary machine.
...
Figure \ref{fig:meshlium} shows example images taken from the supplier web app.

Unfortunately, since these systems are products with financial costs, the intellectual propery is protected and technical specifications are vague and mostly refer to "our processing algorithms". However we can at least see that these products are feasible and obtain a general overview of how they work which will be a useful reference during the design stage.


%=========
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{Meshlium}
\includegraphics[width=0.45\textwidth]{peoplecounting}
\caption{Image of meshlium and retail sensing products taken from supplier website.}
\label{fig:meshlium}
\end{figure}
%=========
...
\begin{enumerate}
  \item Bluetooth / BTLE device scanning - Continuously scans for devices and adds any discoveries to a output map, scans are done for a specified time before restarting to ensure that devices do not stop responding to the scan signal. Algorithm \ref{BTS} details the algorithms used.
...
The system was implemented by using as many extenal libraries as possible, in order to reduce the code required and time taken to develop. A private Github repository was used to manage versions, in addition we use the default Github ticket management service to organise the workload and document progress. We use 3 milestones to seperate the tasks by priority: low priority tasks are assigned to milestone 'Minor', wheras higher priority tasks get assigned to 'Urgent'. The lowest priority is 'Backlog' which holds tasks that are unlikely to be completed as a result of time restraints. We also use the convenient Github wiki service which allows us to document  everything along with source code. We attempt to document as much as possible during implemation as this is a crucial step in producing a realistically usable software product\cite{c-github}.

\section{Smart Sensor Boxes (A-C)}
As stated previously, the smart sensor boxes were implemented using the popular brand of microprocessors, Raspberry Pi (RPi). This product was an excellent choice for a smart sensor box such as this, due to its compatibility with a wide variety of sensors, and its relatively powerful computation ability. Specifically we use the most recent version, “Raspberry Pi 3 Model B” which features inbuilt WiFi and Bluetooth adapters, as well as a dedicated RPi camera port\cite{c-rpi3b}. 
...
In addition we can test the image detection, this is relatively simple as we can just take some images, and access them through VNC Viewer connection to confirm they are correct.
...
\begin{itemize}	
  \item Price - We calculate the standard price of a single smart sensor box to be around \textsterling52, however purchising in bulk would reduce this cost considerably, Appendix B shows the calculations made.
...
\chapter{Web application screen captures (Chrome)}
This appendix shows some screen captures of peoplecount.cf.

Figure \ref{fig:webpage1} shows the home page which displays buildings.
Figure \ref{fig:webpage2} shows the admin page where an admin may modifiy building data.
Figure \ref{fig:webpage3} shows the admin page where an admin may add room information.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{webpage1}
\caption{peoplecount home page}
\label{fig:webpage1}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{webpage2}
\caption{peoplecount admin panel (1/2)}
\label{fig:webpage2}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{webpage3}
\caption{peoplecount admin panel (2/2)}
\label{fig:webpage3}
\end{figure}

\chapter{Smart sensor box pricing}

\begin{enumerate}	
  \item RPi 3 model B - \textsterling30
  \item Standard casing (optional) - \textsterling2
  \item Camera module V2 (optional) - \textsterling20
\end{enumerate}
Therefore the cost range is \textsterling30 - \textsterling52
